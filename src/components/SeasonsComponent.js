...
const SeasonsComponent = function({seasons,showId}){
    let moreActive = seasons.length > 5;
    let moreUrl = '#'
    let seasonArray = limitArray(seasons,5);
        ...
        <div className="container mx-auto px-4 py-16">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8">
                {seasonArray.map((item,key) => {
                    return <SeasonCard season={item} key={key} showId={showId} />
                })}
            </div>
        </div>
    </div>
